        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 16) {
                    ...
                    // 1. Widget Vitalité (Bento Pleine Largeur)
                    VitalityScoreWidget(
                        profile: appState.profile,
                        hrvScore: appState.healthKitManager.latestHrvSDNN
                    )
                    .padding(.horizontal, 20)
                    
                    // 2. Défi Circadien Recommandé
                    DailyChallengeWidget(
                        currentProtocol: appState.recommendedProtocol,
                        onLaunch: onLaunchProtocol
                    )
                    .padding(.horizontal, 20)
                    
                    // 3. Grille Bento 2 Colonnes (Streak & Shield)
                    HStack(spacing: 12) {
                        ...
                        QuickShieldToggleWidget(
                            screenTimeManager: appState.screenTimeManager,
                            onSelectApps: {
                                showAppPicker = true
                            }
                        )
