...
async function initApp() {
  ...
  // Modale d'Architecture & Métriques Système
  const modalSystem = document.getElementById('modal-system-info');
  document.getElementById('btn-open-system-info')?.addEventListener('click', () => {
    modalSystem?.classList.remove('hidden');
    window.audio.playChime(520, 'sine', 0.08);
  });
  document.getElementById('btn-close-system-info')?.addEventListener('click', () => {
    modalSystem?.classList.add('hidden');
    window.audio.playChime(380, 'sine', 0.08);
  });
  modalSystem?.addEventListener('click', (e) => {
    if (e.target === modalSystem) modalSystem.classList.add('hidden');
  });
